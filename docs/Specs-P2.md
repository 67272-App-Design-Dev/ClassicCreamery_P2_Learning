...
9.	have the following scopes:
	...
	- 'is\_18\_or\_older' -- returns all employees 18 years old or older
	- 'younger\_than\_18'  -- returns all employees under 18 years old
...
10.	have the following methods:
	- 'name' -- which returns the employee name as a string "last\_name, first\_name" in that order
	- 'proper\_name' -- which returns the employee name as a string "first\_name last\_name" in that order with a space between them
...
3.	have a store\_id and it must be restricted to stores which exist and are active in the system
4.	have a employee\_id and it must be restricted to employees who exist and are active in the system
5.	have the following scopes:
